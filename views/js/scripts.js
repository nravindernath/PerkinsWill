...
;
		...
		$scope.uploadFile = function(element, elementName) { 
			...
			if(projectSaveType == 'Save') {
				...
				if(!selected_cus_all_users_folder_id) {
					...
					$http.post(clientHost+'/folders/'+selected_cus_box_root_folder_id, fetchFolderIddata, config)
	...
	pwbp.controller('pwteamsetupcontroller', function($scope, $rootScope, AddNewUserscus) {
	...
	pwbp.controller('custeamsetupcontroller', function($scope, $rootScope, AddNewUserscus) {
	...
	pwbp.controller('mainController', function($scope, $rootScope, ngDialog, $timeout, $http,$window,AddNewUserspwt,AddNewUserscus) {
		...
		$scope.changeevt = function (selectedCustomer) {
			...
			selectedcusFolder = selectedCustomer.box_miscelleous_folder_id;
	...
	//----------------------Project Notification-----------------------------------------------				
		...
		var notify = {
			...
			$scope.$setPristine(true);
				...
				$scope.notifydata[$scope.index].link = $scope.link;
...
// date picker script line ------------------------
	...
	if(rowLength > 0) {
		var newTeamMemeber = {
				...
				tile:$scope.pwtmember[$scope.index].tile,
